...
export default function LabTechDashboard() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [pendingRes, completedRes, abnormalRes, todayRes] = await Promise.all([
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <h1 className="text-2xl font-heading font-medium">Laboratory Dashboard</h1>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Pending Tests" value={stats?.pending || 0} icon={<Clock className="h-5 w-5 text-warning" />} />
        <StatCard label="Completed" value={stats?.completed || 0} icon={<CheckCircle className="h-5 w-5 text-primary" />} />
      ...
      <Link to="/laboratory" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded text-sm font-medium hover:bg-primary/90 transition-colors">
        <FlaskConical className="h-4 w-4" /> Go to Laboratory
      </Link>

      <div className="card-ehr p-4">
        <h2 className="font-heading font-medium text-sm mb-3">Pending Test Queue</h2>
        {pendingTests.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">No pending tests.</p>
        ) : (
            ...
            {pendingTests.map((t: any) => (
              <div key={t.id} className="flex items-center justify-between py-2 border-b border-border last:border-0 text-sm">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t.patientName} · {t.test_category || 'General'}</p>
                </div>
